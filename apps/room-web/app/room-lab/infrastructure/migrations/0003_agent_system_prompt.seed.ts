...
/**
 * The default for the `system_prompt` column migration 3 introduces. Seeded
 * identically for every row, because it is a starting point, not a description
 * of anyone in particular: from here it is data the person edits per member
 * on the agents page.
 *
...
/**
 * The data half of migration 3. Order matters: copy prompts from
 * `agent_system_prompts` onto their rows, give rows still without one the
 * default, and only then drop the old table.
 *
